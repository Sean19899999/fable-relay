...
> 派不派**由用户定**——用户开口调用本 skill 即已做完决策（2026-07-12 用户裁定：skill 内不设经济门，"调用即决策"，不许反过来质疑或说教开销）。给人看的"何时值得调用"建议在 README 丑话段，不在这里。
...
- **MUST NOT** 用 SendMessage 组队协作模式；一次派车一个裸代理，跑完即收。
- **合批**：当天攒的多件事**整包写进一份简报派一个代理**（正面范本：2026-07-12 碳会话 5 条意见一包派干完），绝不一件事派一个——每多派一个就多付一次 20-50K 全新上下文底价。
...
- **本 skill 是质量工具不是省钱工具**：缓存损失是接力模式的固有代价，指针化+合批+禁往返只能把浪费压回 20-50K 底价，省不成零。何时值得调用由用户判断（README 丑话段有给人看的建议），skill 不拦。
